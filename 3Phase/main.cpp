...
//void dump_cells(const char *filename, const DiscreteProblem::CellVector& cells,size_t n_iter){
//	std::ofstream strm(filename, std::ios_base::app);
//	strm << "Pb\tRso\tbo\tbw\tbg\tphi\n";
//	strm << "newton iter = " << n_iter << std::endl;
//	for (std::size_t i = 0; i < cells.size(); ++i){
//		strm << cells[i].Pb.value << "\t" << cells[i].Rso.value << "\t" << cells[i].b[0].value 
//			<< "\t" << cells[i].b[1].value << "\t" << cells[i].b[2].value
//			<<  "\t" << cells[i].phi.value;
//	}
//}
...
//   dump_field( "./output/field.out", vPORO, vKX, vKY, vKZ );
   ...
   for (size_t i = 0; i < cumTime.size(); ++i){
	   ...
	   while (time < T_TARGET)
	   {
		   ...
		   ////------##
		   //dump_cells("./output/cellPropers.out", model.mCells, stdsmry.niter);
